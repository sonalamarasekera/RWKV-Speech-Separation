...
class Full_Tmix_Params(nn.Module):
    def __init__(self, n_embd: int, n_layer: int, layer_id: int, head_size: int):
        ...
        for n in range(C):
            linear[n] = n / (C - 1) - 0.5
            zigzag[n] = ((n % head_size) - ((head_size - 1) / 2)) / (
                (head_size - 1) / 2
            )
...
class CGM(nn.Module):
    def __init__(self, n_embd: int):
        super().__init__()
        self.conv = nn.Conv1d(
            2 * n_embd, 2 * n_embd, kernel_size=3, padding=1, groups=1
        )
...
class BiTimeMixFull(nn.Module):
    ...
    def _run_rwkv(self, params, x, v_in, bf16):
        ...
        if RUN_CUDA is not None:
            with torch.amp.autocast(
                "cuda", dtype=torch.bfloat16, enabled=x.is_cuda and bf16
            ):
    ...
    def forward(self, x, v_state, bf16_enabled):
        ...
        y_bwd_rev, v_b_out = self._run_rwkv(
            self.bwd_params, x_rev, v_b_in, bf16_enabled
        )
...
class DualContextAggregation(nn.Module):
    def __init__(self, n_embd: int):
        ...
        self.conv_local = nn.Conv1d(
            n_embd, n_embd, kernel_size=3, padding=1, groups=n_embd
        )
...
class Block(nn.Module):
    def __init__(self, n_embd, n_layer, layer_id, head_size_a, bf16, n_groups):
        ...
        self.tmix = GroupedBiTimeMix(
            n_embd, n_layer, layer_id, head_size_a, n_groups=n_groups
        )
...
class V7Core(nn.Module):
    def __init__(self, n_embd, n_layer, head_size_a, bf16, n_groups):
        ...
        self.blocks = nn.ModuleList(
            [
                Block(n_embd, n_layer, i, head_size_a, bf16, n_groups)
...
class SeparationHead(nn.Module):
    def __init__(
        self, codec_dim: int, hidden: int, num_sources: int, mode: str = "residual"
    ):
        super().__init__()
        self.mode = mode
...
class RWKVv7Separator(nn.Module):
    def __init__(self, cfg: SeparatorV7Config):
        ...
        self.input_proj = nn.Sequential(
            nn.LayerNorm(cfg.codec_dim),
            nn.Linear(cfg.codec_dim, cfg.n_embd, bias=False),
            nn.GELU(),
        )
        self.core = V7Core(
            cfg.n_embd, cfg.n_layer, cfg.head_size_a, cfg.enforce_bf16, cfg.n_groups
        )
        self.output_proj = nn.Sequential(
            nn.LayerNorm(cfg.n_embd),
            nn.Linear(cfg.n_embd, cfg.head_hidden, bias=False),
            nn.GELU(),
        )
        self.head = SeparationHead(
            cfg.codec_dim, cfg.head_hidden, cfg.num_sources, mode=cfg.head_mode
        )
